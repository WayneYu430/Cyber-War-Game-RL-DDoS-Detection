...
class PacketGenerator(object):
    ...
    def __repr__(self):
        return "IP: {}, flow: {}, time_interval: {}".\
            format(self.id, self.flow_id, str(self.adist()))
...
class PacketSink(object):
    ...
    def __init__(self, env, rec_arrivals=False, absolute_arrivals=False, rec_waits=True, debug=False, selector=None):
        ...
        # Record of the packets info for Defender
        self.packet_items = []
    ...
    def put(self, pkt):
        if not self.selector or self.selector(pkt):
            ...
            if pkt.flow_id == 'attacker':
                flow_id_i = 1
            else:
                flow_id_i = 0
            self.packet_items.append([pkt.src, pkt.size, flow_id_i]) # src_ip, bytes, flow_id
            if self.debug:
                print(pkt)

    # Handle the queue only once
    def get_pkt_items(self,):
        tmp_list = self.packet_items
        self.packet_items = []
        return tmp_list
...
class SwitchPort_Defend(object):
    """ 
    ...
    """
    def __init__(self, env, rate, qlimit=None, limit_bytes=True, debug=False,if_drop=None):
        self.store = simpy.Store(env)
        self.rate = rate
        self.env = env
        self.out = None
        ...
        self.byte_dropped = 0   # Used to collect the dropped bytes
        self.action = env.process(self.run())  # starts the run() method as a SimPy process

    def run(self, if_drop=None):
        while True:
            # print(count, ": Get self.store.items", len(self.store.items))
            msg = (yield self.store.get())  # The Pkt object from FIFO Queue
            self.busy = 1
            self.byte_size -= msg.size
            yield self.env.timeout(msg.size*8.0/self.rate)
            # print("Self.msg", msg.src)
            self.out.put(msg)   # put into the PS
            self.busy = 0
            if self.debug:
                print("Defender: ",msg)
 
    def put(self, pkt, if_drop=None):   # Call at PG.out.put()...
        """
        ...
        if pkt.flow_id == "user":
            print("DROP USER")
        elif pkt.flow_id == "attacker":
            print("ATTACKER")
            self.packets_rec += 1
        """
        # print("IN defender, finding flow %s" %(pkt.flow_id))
        if if_drop is not None:
            # print("IN defender, finding flow %s, Dropping" %(pkt.flow_id))
            self.packets_rec += 0
            self.packets_drop_by_agent += 1
            self.byte_dropped += pkt.size
            tmp_byte_count = self.byte_size + 0
        else:
            # print("IN defender, finding flow %s, Not Drop" %(pkt.flow_id))
            self.packets_rec += 1
            tmp_byte_count = self.byte_size + pkt.size
        ...
        if self.qlimit is None:
            self.byte_size = tmp_byte_count
            return self.store.put(pkt)
        if self.limit_bytes and tmp_byte_count >= self.qlimit:  # Exceed the memory
            print("IN defender, Exceeding the Queue Limitation")
            self.packets_drop += 1
            return
        elif not self.limit_bytes and len(self.store.items) >= self.qlimit-1:
            self.packets_drop += 1
