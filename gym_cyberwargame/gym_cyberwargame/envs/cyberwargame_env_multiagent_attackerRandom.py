...
class CyberWarGameEnvPettingZoo(AECEnv):
    ...
    def __init__(self, env_args=env_args):
        ...
        self.attacker_action_space = spaces.Box(low=np.array([0] * host_num), high=np.array([a_action] + [0]*(host_num-1)), \
        ...
        self.defender_action_space = spaces.Box(low=0, high=1, shape=(self.bot_net_num + self.user_num,), \
                                                dtype=np.int32)  # 0 drop and 1 not drop
    ...
    def step(self, action):
        ...
        defend_action = None
        if agent == "attacker":
            # attack_action = action[0]
            attack_action_ip = random.randint(0, 1)
        ...
        if agent == "attacker":
            ...
            if self.keep_util and self.keep_util_count >= 10:
                ...
                self.cbw_logger.logger.info(
                    "The game Done with Attacker {}, with utilization keep {}".format(self.rewards, self.keep_util_count))
            ...
            if self.step_num_test >= 400:
                ...
                self.cbw_logger.logger.info(
                    "The game Done with Defender {}, with utilization keep {}".format(self.rewards, self.keep_util_count))
                return self._get_obs(), self.rewards, self.dones, self.infos
            # Attack Reward
            ...
            # Defender Reward
            """
            1, if packet is legitimate and it passed
            1, if packet is attack and it was dropped
            -1, if packet is legitimate and it was dropped
            -1, if packet is attack and it was passed
            -10, if utilization exceed threshold
            """
            """
            ...
            """
            # 0815 Test for range of defender use
    ...
    def _get_obs(self, agent=None, attack_ack=None):
        ...
        if agent and agent == 'attacker':
            ...
            return {"attacker": np.array(attack_list, dtype=np.float32), "defender": np.array(defender_list,  dtype=np.float32)}
        # return attack_list
